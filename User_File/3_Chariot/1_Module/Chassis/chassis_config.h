...
typedef struct
{
     ...
     *
     * ⚠⚠ 两条 2026-09-12 定下的结论，判断"要不要保留这个屏障"时必须先读：
     *
     * 1) 装上气弹簧之后静止姿态会被挤到 phi0≈0 和 ≈3.14 两极，而 phi0≈0 正好在
     *    屏障【下侧】——所以屏障会从偶尔触发变成【常态触发】，它的正确性变得
     *    更要紧，而不是像直觉那样更不要紧。
     *    机理：弹簧本身不产生 Tp（膝角只是 L0 的函数，见 Chassis_Spring_Config_t），
     *    但它把腿顶到最长之后，地面反力经由这条长腿产生了 Tp——竖直反力 N 对髋
     *    关节的力矩是 N*L0*sin(theta)，方向是让 theta 继续【偏离】竖直。所以
     *    无电机力矩时 phi0≈0.9 这类中间角是不稳定平衡，腿必然滑向两极。
     *
     * 2) 屏障是 58c8788（"加入卡腿绕远路机制，验证通过"）加的，但那次验证跑在
     *    APP_DM_TOR_MIN/MAX = ±15 而电机 TMAX = 40 的错配下，【关节力矩是命令值
     *    的 2.67 倍】，卡腿比现在猛得多。所以"从正前方往下转会把腿怼进地里"这个
     *    机械前提在正确标度下是否还成立，需要重测（台架测法见 实机调试检查清单 §2.5）。
     *
     * 摆腿那一半的判据另有一个已确认的帧不一致缺陷，详见 chassis_control.c 的
     * Recovery_Swing 方向锁注释。
     */
    float phi0_barrier;

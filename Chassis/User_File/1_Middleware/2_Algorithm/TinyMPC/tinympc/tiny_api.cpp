...
static int check_dimension(std::string matrix_name, std::string rows_or_columns, int actual, int expected) {
    if (actual != expected) {
        /* [本工程改动] 去打印 */ /* std::cout << matrix_name << " has " << actual << " " << rows_or_columns << ". Expected " << expected << "." << std::endl; */
        return 1;
    }
    return 0;
}
...
int tiny_set_bound_constraints(TinySolver* solver,
    ...
    if (!solver) {
        /* [本工程改动] 去打印 */ /* std::cout << "Error in tiny_set_bound_constraints: solver is nullptr" << std::endl; */
        return 1;
    }
...
int tiny_set_cone_constraints(TinySolver* solver,
    ...
    if (!solver) {
        /* [本工程改动] 去打印 */ /* std::cout << "Error in tiny_set_cone_constraints: solver is nullptr" << std::endl; */
        return 1;
    }
...
int tiny_set_linear_constraints(TinySolver* solver,
    ...
    if (!solver) {
        /* [本工程改动] 去打印 */ /* std::cout << "Error in tiny_set_linear_constraints: solver is nullptr" << std::endl; */
        return 1;
    }
...
int tiny_precompute_and_set_cache(TinyCache *cache,
    ...
    if (!cache) {
        /* [本工程改动] 去打印 */ /* std::cout << "Error in tiny_precompute_and_set_cache: cache is nullptr" << std::endl; */
        return 1;
    }
    ...
    // Printing
    if (verbose) {
        /* [本工程改动] 去打印 */ /* std::cout << "A = " << Adyn.format(TinyApiFmt) << std::endl; */
        /* [本工程改动] 去打印 */ /* std::cout << "B = " << Bdyn.format(TinyApiFmt) << std::endl; */
        /* [本工程改动] 去打印 */ /* std::cout << "Q = " << Q1.format(TinyApiFmt) << std::endl; */
        /* [本工程改动] 去打印 */ /* std::cout << "R = " << R1.format(TinyApiFmt) << std::endl; */
        /* [本工程改动] 去打印 */ /* std::cout << "rho = " << rho << std::endl; */
    }
    ...
    for (int i = 0; i < 1000; i++)
    {
        ...
        {
            if (verbose) {
                /* [本工程改动] 去打印 */ /* std::cout << "Kinf converged after " << i + 1 << " iterations" << std::endl; */
            }
            break;
        }
        Ktp1 = Kinf;
    ...
    if (verbose) {
        /* [本工程改动] 去打印 */ /* std::cout << "Kinf = " << Kinf.format(TinyApiFmt) << std::endl; */
        /* [本工程改动] 去打印 */ /* std::cout << "Pinf = " << Pinf.format(TinyApiFmt) << std::endl; */
        /* [本工程改动] 去打印 */ /* std::cout << "Quu_inv = " << Quu_inv.format(TinyApiFmt) << std::endl; */
        /* [本工程改动] 去打印 */ /* std::cout << "AmBKt = " << AmBKt.format(TinyApiFmt) << std::endl; */
        /* [本工程改动] 去打印 */ /* std::cout << "APf = " << APf.format(TinyApiFmt) << std::endl; */
        /* [本工程改动] 去打印 */ /* std::cout << "BPf = " << BPf.format(TinyApiFmt) << std::endl; */
...
int tiny_update_settings(TinySettings* settings, tinytype abs_pri_tol, tinytype abs_dua_tol,
    ...
    if (!settings) {
        /* [本工程改动] 去打印 */ /* std::cout << "Error in tiny_update_settings: settings is nullptr" << std::endl; */
        return 1;
    }
...
int tiny_set_default_settings(TinySettings* settings) {
    if (!settings) {
        /* [本工程改动] 去打印 */ /* std::cout << "Error in tiny_set_default_settings: settings is nullptr" << std::endl; */
        return 1;
    }
...
int tiny_set_x0(TinySolver* solver, tinyVector x0) {
    if (!solver) {
        /* [本工程改动] 去打印 */ /* std::cout << "Error in tiny_set_x0: solver is nullptr" << std::endl; */
        return 1;
    }
...
int tiny_set_x_ref(TinySolver* solver, tinyMatrix x_ref) {
    if (!solver) {
        /* [本工程改动] 去打印 */ /* std::cout << "Error in tiny_set_x_ref: solver is nullptr" << std::endl; */
        return 1;
    }
    int status = 0;
...
int tiny_set_u_ref(TinySolver* solver, tinyMatrix u_ref) {
    if (!solver) {
        /* [本工程改动] 去打印 */ /* std::cout << "Error in tiny_set_u_ref: solver is nullptr" << std::endl; */
        return 1;
    }
    int status = 0;

...
static void testFeedbackRoutingAndMapping(void)
{
    ...
    Motor_DM_Init();
    for (motor_id = 1U; motor_id <= MOTOR_DM_COUNT; motor_id++)
    {
        motor_dm_state_t state = {0};
        float direction = (motor_id <= 2U) ? 1.0f : -1.0f;

        test_tick++;
        makeFeedback((uint8_t)motor_id, 1U, 1.0f, 3.0f, 2.0f, data);
        Motor_DM_UpdateFeedback(APP_CAN_BUS_FDCAN1,
                                APP_DM_FB_MASTER,
                                data);
        Motor_DM_GetState(indices[motor_id - 1U], &state);
        assert(state.feedbackCount == 1U);
        assert(state.state == 1U);
        assert(state.mosTemperature == 51U);
        assert(state.rotorTemperature == 52U);
        assertNear(state.positionWrappedRad, direction * 1.0f);
        assertNear(state.positionRad, direction * 1.0f);
        assertNear(state.velocityRadps, direction * 3.0f);
        assertNear(state.torqueNm, direction * 2.0f);
    }
    ...
    {
        motor_dm_state_t state = {0};
        Motor_DM_GetState(MOTOR_DM_LEFT_BACK, &state);
        assert(state.feedbackCount == 1U);
    }

    Motor_DM_Init();
    makeFeedback(5U, 1U, 0.0f, 0.0f, 0.0f, data);
    Motor_DM_UpdateFeedback(APP_CAN_BUS_FDCAN1,
                            APP_DM_FB_MASTER,
                            data);
    for (motor_id = 0U; motor_id < MOTOR_DM_COUNT; motor_id++)
    {
        motor_dm_state_t state = {0};
        Motor_DM_GetState((motor_dm_index_t)motor_id, &state);
        assert(state.feedbackCount == 0U);
    }
}

static void testPositionUnwrapAndTimeout(void)
{
    uint8_t data[APP_DM_FRAME_LEN];
    motor_dm_state_t state = {0};
...
static void testTorqueOnlyTransmit(void)
{
    ...
    uint16_t expected_raw = floatToUint(1.0f,
                                          APP_DM_TOR_MIN,
                                          APP_DM_TOR_MAX,
                                          12U);
    uint32_t index;
...
static void testEnableThenSafeZeroTransmit(void)
{
    motor_dm_command_t command = {.torqueNm = 1.0f};
...
int main(void)
{
    testFeedbackRoutingAndMapping();
    testPositionUnwrapAndTimeout();
    testTorqueOnlyTransmit();
    testEnableThenSafeZeroTransmit();
    return 0;
}

...
void Motor_DM_Init(void)
{
    ...
    /*
     * 位置量程与 Real 工程保持一致：两台电机的 PMAX 都已在调试助手里设成 3.1416。
     * 取 pi 而不是出厂的 12.5，是因为 12.5 的定义域接近两圈，和
     * Algorithm_AngleUnwrapRad 的 ±pi 假设对不上，位置回绕一次就注入 0.133 rad 误差。
     * 两个工程跑的是同两台电机，这两项必须跟 Real 一起改。
     */
    dmMotors[MOTOR_DM_YAW].config = (motor_dm_config_t){
        .bus = APP_CAN_BUS_FDCAN1,

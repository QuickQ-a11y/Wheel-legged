...
#define GIMBAL_FAULT_REMOTE 0x100U  /* 遥控离线或急停。 */

/** @brief 云台外层状态。 */
typedef enum
{
    GIMBAL_ZERO_FORCE = 0, /* 无力，两路输出恒零。 */
    GIMBAL_MANUAL,         /* 遥控手动。 */
    GIMBAL_VISION,         /* 视觉自瞄。 */
    GIMBAL_IDENT,          /* 系统辨识激励。 */
} Gimbal_State_t;
...
typedef struct
{
    float angle;      /* 关节角，单位 rad，已含 scale*ratio 和零位。 */
    float d_angle;    /* 关节角速度，单位 rad/s。 */
    float T_fb;       /* 电机反馈力矩，单位 N*m。 */
    float target;     /* 关节角目标，单位 rad。 */
    float T_req;      /* 控制器请求力矩，单位 N*m；故障时仍然计算。 */
    float T;          /* 安全门后实际下发的力矩，单位 N*m。 */
    uint8_t online;   /* 电机在线。 */
} Gimbal_Axis_t;

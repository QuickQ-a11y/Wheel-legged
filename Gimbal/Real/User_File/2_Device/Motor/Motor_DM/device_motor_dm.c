 ...
 *
 * 刻意不写 default 分支：漏掉某个总线时编译器会以 -Wswitch 报警，而不是像原来
 * 那样静默落到 hfdcan1。云台电机从 FDCAN1 移到 FDCAN3 时就是栽在这个静默兜底上，
 * 配置改了、这里没改，使能帧全发到了空总线，电机永远不上报。
 */
...
/**
 * @brief 初始化云台两台 DM 的 CAN 映射并复位命令与安全状态。
 */
void Motor_DM_Init(void)
{
    ...
    /*
     * 两台都取 ±pi 而不是出厂的 ±12.5：YAW 会连续转，12.5 的定义域接近两圈，和
     * Algorithm_AngleUnwrapRad 的 ±pi 假设对不上——实机确认过 positionWrappedRad
     * 涨到 +12.5 会直接跳到 -12.5，每跳一次展开就注入 0.133 rad 误差并累积。
     * PITCH 行程只有 ±0.5 rad、一次都不会回绕，取 pi 纯属白赚 4 倍分辨率。
     * 改这里必须同步改调试助手里对应电机的 PMAX，两边不一致位置会整体缩放。
     */
    dmMotors[MOTOR_DM_YAW].config = (motor_dm_config_t){
        .bus = APP_CAN_BUS_FDCAN3,
        .commandId = APP_DM_YAW_ID,
        .feedbackId = APP_DM_YAW_FB,
        .positionMin = -3.14159265f,
        .positionMax = 3.14159265f,
    };
    dmMotors[MOTOR_DM_PITCH].config = (motor_dm_config_t){
        .bus = APP_CAN_BUS_FDCAN3,
        .commandId = APP_DM_PITCH_ID,
        .feedbackId = APP_DM_PITCH_FB,
        .positionMin = -3.14159265f,
        .positionMax = 3.14159265f,
    };
    ...
    for (index = 0U; index < MOTOR_DM_COUNT; index++)
    {
        ...
        float wrappedPositionRad =
            Motor_DM_UintToFloat(positionRaw,
                                 motor->config.positionMin,
                                 motor->config.positionMax,
                                 16U);
